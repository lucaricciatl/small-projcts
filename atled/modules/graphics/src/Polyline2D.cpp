...
Polyline2D::Polyline2D(raylib::Color color) : color(color) {}
...
void Polyline2D::Draw() const {
  ...
  for (size_t i = 0; i < points.size() - 1; ++i) {
    raylib::DrawLine(points[i][0], points[i][1], points[i + 1][0],
                     points[i + 1][1], color);
  }
}
...
void Polyline2D::SetColor(raylib::Color newColor) { color = newColor; }
